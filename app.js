(function () {
  ...
  function setServerInfo(data) {
    ...
    if (data.players !== undefined && data.players !== null) playerCount.textContent = data.players;
    if (data.maxPlayers !== undefined && data.maxPlayers !== null) maxPlayers.textContent = data.maxPlayers;
  }

  function setPlayerName(name) {
    var clean = String(name || "").trim();
    playerName.textContent = clean || "Agent";
  }
  ...
  // Appelée automatiquement par Garry's Mod.
  window.GameDetails = function (server, url, map, max, steamId, gamemode) {
    ...
    // Le nom RP n'est pas transmis nativement par GMod au loading screen.
    // Une petite API externe peut le retrouver à partir du SteamID64.
    ...
    // Le nombre actuel de joueurs n'est pas fourni par GameDetails.
    // Il peut venir d'une API de statut optionnelle.
    if (cfg.serverStatusApiUrl) fetchJson(cfg.serverStatusApiUrl, setServerInfo);
  };

  window.SetFilesTotal = function (total) { totalFiles = Number(total) || 0; };
  window.SetFilesNeeded = function (needed) {
    neededFiles = Number(needed) || 0;
    if (totalFiles > 0) setProgress(((totalFiles - neededFiles) / totalFiles) * 100);
  };
  window.DownloadingFile = function (fileName) { detailText.textContent = "Transfert : " + fileName; };
  window.SetStatusChanged = function (status) { if (status) detailText.textContent = status; };

  if (cfg.simulatedLoading !== false) {
    var simulation = setInterval(function () {
      if (receivedGameDetails || totalFiles > 0) { clearInterval(simulation); return; }
  ...
  if (ctx) {
    ...
    function resize() {
      ...
      for (var i = 0; i < count; i++) particles.push({
        ...
        r: Math.random() * 1.2 + .25, vy: -(Math.random() * .12 + .025),
        vx: (Math.random() - .5) * .08, a: Math.random() * .26 + .04
      });
    }
    function draw() {
      ...
      for (var i = 0; i < particles.length; i++) {
        ...
        ctx.fillStyle = "rgba(222, 205, 170," + p.a + ")"; ctx.fill();
      }
